...
**agent-device**:
Upstream Callstack CLI that inspects, controls, and verifies apps on device/simulator targets. Installed by `skills/xq-device/scripts/install-cli.sh`. Agents read its installed `help` topics for command contracts.
_Avoid_: Our CLI (say agent-device or a future package name explicitly)

**Skill**:
A folder under `skills/<name>/` with `SKILL.md` (Agent Skills format) and optional `scripts/`. Validated/published with `gh skill publish`. Thin routers point at the bundled install script + version-matched `agent-device help …`.
